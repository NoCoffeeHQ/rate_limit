require 'spec_helper'
RSpec.describe RedisRateLimit do
  describe Onyx::RateLimit do
    let(:instance) { described_class.new('MyTask', max_rps: 3, redis: Onyx.redis) }
    it 'accepts to pass a splat keyword arguments' do
      settings = { max_rps: 1, redis: Onyx.redis }
      expect { described_class.new('LocationIQ', **settings) }.not_to raise_error
    end
    describe '#safe_call' do
      ...
      context '2 calls within a second, delay of 1s, 2 more calls (exceeding the rpm limit), delay of 1s, one more call (exceeding the rpm limit)' do
        let(:instance) { described_class.new('MyTask', max_rps: 2, max_rpm: 3, redis: Onyx.redis) }
